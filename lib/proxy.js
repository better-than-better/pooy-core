...
const { writeToLocalAsync, resetTmpDir } = require('./utils');
...
const IGNORE_HOSTS = Symbol('ignore-hosts');
...
class Proxy extends EventEmitter {
  constructor(props) {
    ...
    this.on('request', (context) => {
      context.setTime('request');
      this.setContext(context);
    });
    this.on('requestEnd', (context) => {
      context.setTime('requestEnd');
    });
    this.on('response', (context) => {
      context.setTime('response');
    });
    this.on('responseEnd', (context) => {
      context.setTime('responseEnd');
    });
  ...
  isIgnoreHost(host) {
    ...
    this.ignoreHosts.forEach(v => {
      if ((new RegExp(v)).test(host)) {
        ignore = true
      }
    });

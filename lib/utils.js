...
const createFromRootCA = require('../worker/create-from-root-ca');
...
exports.getSelfSignCert = async (hostname) => {
  const sslDir = `${pooyDir}/ssl`;

  if (!fs.existsSync(sslDir)) {
    fs.mkdirSync(sslDir);
  }

  const filePath = `${sslDir}/${hostname}`;
  let caData = {};

  try {
    caData = JSON.parse(fs.readFileSync(filePath));
  } catch(e) {
    caData = await createNewCA(hostname);
  }

  async function createNewCA(hostname) {
    const caData = await createFromRootCA(hostname, 2048);

    fs.writeFile(filePath, JSON.stringify(caData), () => {});
    return caData;
  }
